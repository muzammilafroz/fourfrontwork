    ...
    if (search.trim() === "") {
      ...
      const filtered = requests.filter(
        (req) =>
          ...
          (req.composition &&
            req.composition.toLowerCase().includes(searchTerm)) ||
          (req.customer_name &&
            req.customer_name.toLowerCase().includes(searchTerm)) ||
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage customer medicine requests and track order status
          </p>
        </div>
      </div>
