...
const API = "http://localhost:8000/api/overview";
const COLORS = [
  "#2A9D8F",
  "#457B9D",
  "#E9C46A",
...
const Overview = () => {
  ...
  const loadData = async (showRefreshToast = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.detail || "Failed to load overview");
      }
  ...
  const hasDoctorVisitsData =
    charts.doctorVisits && charts.doctorVisits.length > 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {statCards.map((s) => (
          <div
            key={s.label}
            className="bg-card rounded-xl border border-border p-3 hover:border-primary/50 transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* TOP MEDICINE DEMAND */}
        <ChartCard
          title="Top Medicine Demand"
          subtitle="Most requested medicines"
        >
          {hasDemandData ? (
            <ResponsiveContainer width="100%" height={250}>
              <BarChart
        ...
        <ChartCard title="Doctor Visits" subtitle="Visit statistics">
          {hasDoctorVisitsData ? (
            ...
            <ChartEmptyState
              message="No doctor visits data available"
              icon={<Activity className="h-8 w-8" />}
            />
          )}
        </ChartCard>
