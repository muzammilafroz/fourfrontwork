...
const API = "http://localhost:8000/api/medicines";
...
const Inventory = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(editMed ? `${API}/${editMed.id}` : API, {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API}/${deleteMed.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage medicines, track stock levels, and monitor expiries
          </p>
        </div>
        <Button
          onClick={() => {
